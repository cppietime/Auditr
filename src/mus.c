...
double rectsin(double arg){
	return abs(sin(arg*2*M_PI));
}

double square(double arg){
	return (abs(arg-(int)arg)<.5)?1.0:-1.0;
}
...
double logmap(int max){
	...
	return ((int)(logmap_x*max))%max;
}

wavegen drivers[] = {wavesin, sawtooth};
...
int main(){
	...
	make_vowels(srate);
	init_instrs();
	track* master = gen_master_track(uni_meas, bpm, logmap);
	track* play_track = gen_play_track(mpb, master, logmap);
	FILE* shid = fopen("shidi.shid","wb");
	init_file(shid,srate,basefreq,bps,bpm*mpb);
	write_track(shid, play_track, logmap);
	fclose(shid);
